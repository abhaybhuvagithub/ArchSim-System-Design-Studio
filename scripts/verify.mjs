  ...
  // ── read aloud ─────────────────────────────────────────────────────────────
  {
    ...
    // ── the ROI tab, driven ──────────────────────────────────────────────────
    {
      ...
      check('Engineering Leadership teaches technical judgment (one-way doors, build-vs-buy) not people-management', await (async () => {
        ...
        const allText = el.items.map(i => i.d).join(' ');
        return !!doors && /irreversible/i.test((doors.cols.join(' ') + ' ' + doors.rows.map(r => r.join(' ')).join(' ')))
          && !!bba && bba.cols.length === 3 && /differentiator/i.test(bba.rows.map(r => r.join(' ')).join(' '))
          && !/attrition|morale|promotion advocacy/i.test(allText);
      })());
