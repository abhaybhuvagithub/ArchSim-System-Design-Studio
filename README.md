...
- **Template library — 68 pre-wired designs**, each with its own requirements checklist:
  - *Product designs:* **News Feed (Instagram)** (hybrid fan-out, media transcode pipeline, ranking on the hot path), **Music Streaming (Spotify)** (per-market licensing on every play, immutable audio, royalty events that cannot be lost), **Distributed File Storage (Drive)** (content-hashed chunk dedup, inherited ACLs with cache invalidation, change-log sync), **Redis (Distributed Cache)** (hash-slot sharding, async replication and failover, AOF/RDB persistence, one cluster serving a cache, a rate limiter, a leaderboard and pub/sub), **Windy** (pre-rendered weather map tiles, geospatial indexing, model ensemble aggregation, separate websocket and REST lanes), **AccuWeather** (API quota enforcement off the critical path, per-tier SLO separation, usage metering for billing), Bitly, Ticketmaster, Uber, YouTube, WhatsApp, Web Crawler, Google Docs, Twitter feed, Dropbox, Rate Limiter, Yelp, Leaderboard, Notifications, Payments, Autocomplete.
  - *Microservice architecture patterns:* E-commerce with Saga / database-per-service, CQRS + Event Sourcing, BFF + service-mesh platform, **Cloud-Native Gateway API Platform** (Kubernetes Gateway API with native canary traffic splitting, a single gRPC-JSON transcoder at the edge, mesh-terminated mTLS, and a token-aware AI-assistant lane split from the REST/gRPC data plane), event-driven choreography with DLQ, and Strangler-fig monolith migration with CDC.
  ...
  - *Robotics & edge:* **Simbe Tally** — an autonomous shelf-scanning robot fleet: on-robot NVIDIA inference, constrained store uplink, billions of write-once shelf images, embedding + ANN product recognition across millions of SKUs, retailer planogram/POS integration, and a task engine that turns detections into "restock aisle 7" for associates. **Tesla Ecosystem** — on-vehicle Autopilot shadow-mode inference so only flagged edge cases leave the car, batch model training feeding a staged, reversible OTA rollout, and a completely separate money-critical lane for remote commands and the Supercharger network.
  ...
  - *AI / ML:* **Anthropic Claude** (API keys and org auth, token-based rate limits, input/output safety, prompt cache, model router, GPU scheduler and inference fleet, tool use/MCP, exact usage metering — $184 per million requests, all of it inference), GenAI RAG assistant (embedder, vector DB, semantic cache, LLM inference, in/out guardrails, eval traces) and a two-stage recommendation ranker (candidate generation → feature store → ranker, with a feedback loop and model registry). **Agentic AI:** **Multi-Agent Orchestration Platform** (a supervisor decomposing work across concurrent specialist agents, a shared versioned blackboard instead of private memory, a human-in-the-loop gate for irreversible actions, one trace across every agent in a run) and **Autonomous Coding Agent** (plan-execute-verify in an isolated, checkpointed sandbox with a real test runner, hard-blocking guardrails in front of the shell, and a human review gate before anything merges).
