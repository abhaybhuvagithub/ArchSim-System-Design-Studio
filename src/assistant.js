...
export function offlineAnswer(question, { nodes = [], edges = [], sim, cost, sugs = [], faults = [], rps, simOn, template }) {
  ...
  const GLOSSARY = {
    ...
    'prakriti': 'In Ayurveda, an individual\'s constitutional type assessed across the doshas. In the Ayurveda Gyaan template it is the structured, dual-coded output of an assessment questionnaire - designed to travel into a modern EHR via NAMASTE and ICD-11 TM2 codes without translation loss.',
    'shloka addressing': 'Canonical verse identity as sthana.chapter.verse - the primary key of a civilization-scale text. Commentaries and translations attach to the id as versioned overlays; the source is never edited. The trap is edition variance, which is why the concordance table exists.',
