...
export function buildReport(ctx) {
  ...
  // ---- 6b. what to fix, and why -----------------------------------------
  const health = diagnoseAll(cap, nodes, sim, fx, faults)
  if (health.length) {
    sections.push(sec('Needs attention', {
      intro: `${health.length} tier${health.length > 1 ? 's need' : ' needs'} work before this design is production-ready. Each one is a single edit inside ArchSim.`,
      table: {
        cols: ['Tier', 'Problem', 'Why it matters', 'Fix'],
        rows: health.map(d => [
          d.row.label,
          { down: 'Offline', critical: 'Over capacity', warn: 'Bottleneck', spof: 'Single point of failure' }[d.level],
          d.why,
          d.fix.label,
        ]),
      },
  ...
  return {
    ...
    footer: `Generated by ArchSim — System Design Studio · ${template ? template.name : 'Untitled design'} · ${now.toLocaleDateString()}`,
    sections,
  }
}
