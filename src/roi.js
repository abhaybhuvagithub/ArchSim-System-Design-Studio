...
const AUTHORED = {
  ...
  'Billing & Revenue Platform': { revPerM: 20000, model: 'the system that collects ALL other revenue', basis: 'A billing platform rarely sells on its own - its ROI is that it is the mechanism through which every other dollar is collected, so its value is measured in leakage prevented and cash accelerated. Billed-but-not-collected is pure lost revenue, and studies of subscription businesses put involuntary churn (failed payments never recovered) at a few percent of revenue - which dunning and smart retries claw back directly. Blended, a million billing-pipeline operations carries roughly $15-25K in attributable value, but the honest framing is defensive: a double-billing bug refunds and churns customers, a tax error is a fine, and revenue recognized wrong is a restatement. The dominant cost is correctness engineering and audit, because here a bug is a wrong number in someone\'s financial statements, not a slow page.' },
  'Agent Platform (Durable Execution)': { revPerM: 45000, model: 'platform/seat + metered agent-runs', basis: 'An agent platform is sold as infrastructure - a platform fee plus metering on agent-runs and the compute/tokens they consume - so blended across control-plane and workflow traffic a million operations carries roughly $40-50K. But the honest ROI framing is defensive as much as revenue: the platform\'s value is that autonomous agents do real work WITHOUT the incidents unmediated autonomy causes - a double-charge from a crash-retry, a destructive tool call, a reasoning loop that burns the budget. The dominant cost is the model calls the agents make (the same LLM-gateway economics as any AI system), and the guarantees - durability, permissions, approval, budget - are what let a business trust agents with real actions, which is the entire product.' },
  ...
  'PostgreSQL at Scale (Primary + Replicas)': { revPerM: 0, model: 'infrastructure - measured in what it prevents', basis: 'PostgreSQL is open-source infrastructure, not a product with a top line, so revenue-per-request is the wrong lens - the value is what a correct, durable, well-scaled database prevents: lost writes, stale-read bugs shipped to users, and the outage when connection count quietly takes the primary down. The dominant real cost is operational - replica fleet, backup storage, and the engineering time to run failover and PITR properly - and the honest ROI framing is availability-and-correctness per dollar: an unpooled connection storm or a missing PITR plan is far more expensive than the replicas and backups that would have prevented it. The cheapest database is the one that never loses an acknowledged write.' },
