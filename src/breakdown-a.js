...
'URL Shortener (Bitly)': {
  ...
  overview: 'Take a long URL, hand back a short one, and redirect billions of times a day. The write path is trivial; everything interesting lives in the read path and in generating codes that never collide.',
  scope: 'This problem is deceptively simple, so precision is what distinguishes a good answer. Keep analytics and user accounts below the line and spend the time on code generation and the redirect path — that is where an interviewer is actually probing.',
  planning: 'Establish the read:write ratio early, because roughly 100:1 justifies every subsequent decision. Then settle short-code generation, explaining why hashing and random-with-retry are worse than a counter. Only after that reach for caching, and be explicit that the redirect path should barely touch a database.',
  fr: {
    core: ['Shorten a long URL into a short code', 'Support custom aliases and an expiry', 'Redirect a short code to the original URL', 'Report basic click counts'],
    out: ['User accounts and dashboards', 'Link preview generation', 'Malware and abuse scanning'],
  },
  nfr: {
    core: ['Redirect p99 under 100ms', '99.99% availability — a dead redirect breaks every link ever shared', 'Codes unique and never reused', 'Read-heavy, roughly 100:1'],
    out: ['Strong consistency on click analytics'],
  },
  nums: [['1B', 'redirects/day'], ['~12K/s', 'average redirect QPS'], ['~115/s', 'new links (10M/day)'], ['~2 TB', 'storage over five years']],
  entities: [
    ['Link', 'shortCode → longUrl, plus owner, createdAt and expiresAt'],
    ['Click', 'an analytics event: code, timestamp, referrer, geography'],
    ['Counter', 'the monotonic source of new codes, handed out in blocks'],
  ],
  apiIntro: 'Plain REST is right here. Writes are infrequent and the read is a single GET the browser issues by itself.',
  api: [
    { dir: '→', name: 'POST /urls', body: '{ longUrl, customAlias?, expiresAt? }\n→ 201 { shortUrl }' },
    { dir: '→', name: 'GET /{shortCode}', body: '→ 302 Found\nLocation: <longUrl>' },
    { dir: '→', name: 'GET /urls/{code}/stats', body: '→ { clicks, byDay[], byCountry[] }' },
  ],
  apiNote: 'Use 302, not 301. A permanent redirect lets browsers cache forever, which destroys your analytics and makes a link impossible to revoke.',
  dives: [
    {
      title: 'How do you generate a short code?', focus: ['app', 'db'],
      blocks: [['p', 'The whole design hinges on this. Three candidates, and the reasoning matters more than the answer.']],
      options: [
        { rating: 'Bad', title: 'Random code, check for collisions', approach: 'Generate seven random base62 characters and check the database before inserting.', challenges: 'Every write costs an extra read, and the retry rate climbs as the keyspace fills. Two concurrent writers still race unless the insert is conditional.' },
        { rating: 'Good', title: 'Hash the long URL and truncate', approach: 'MD5 or SHA the URL and take the first seven base62 characters.', challenges: 'Truncation reintroduces collisions, and identical URLs from different users collapse to one code — which breaks per-user analytics and expiry.' },
        { rating: 'Great', title: 'Base62-encode a counter', approach: 'Collisions are impossible by construction. Hand each instance a block of a thousand ids so the counter is off the hot path entirely.', challenges: 'Codes become sequential and therefore enumerable. Permute the counter with a bijective function before encoding — you keep uniqueness and lose the guessability.', best: true },
      ],
    },
    {
      title: 'Surviving a link that goes viral', focus: ['cache', 'cdn', 'app'],
      blocks: [
        ['p', 'One link can take a single cache shard to saturation on its own. Three mitigations, cheapest first: replicate the hot key across shards and read one at random; keep a small in-process LRU on each redirect server so the hottest few thousand codes never leave the box; or let the CDN absorb it entirely.'],
        ['note', 'The in-process cache is the highest-leverage fix — it removes a network hop and costs a few megabytes of RAM.'],
      ],
    },
    {
      title: 'Analytics without slowing the redirect', focus: ['db', 'app'],
      blocks: [
        ['p', 'Never write analytics synchronously in the redirect path. Emit a click event and aggregate it elsewhere. Exact counts are not worth the latency, and approximate counters are entirely adequate for a stats page.'],
        ['warn', 'This is a common trap: a single counter increment on the redirect path can double your p99 and couples your most-available endpoint to your least-available store.'],
      ],
    },
  ],
  bar: {
    mid: 'A working read/write split with a cache, and a code scheme you can defend. Expect to be asked why 302 rather than 301.',
    senior: 'Reach the counter scheme quickly and explain precisely why the alternatives fail. Raise the hot-key problem before being asked.',
    staff: 'Cover counter block allocation and what happens when an instance dies holding an unused block, multi-region reads, and how a link is revoked globally within seconds despite edge caching.',
  },
},
