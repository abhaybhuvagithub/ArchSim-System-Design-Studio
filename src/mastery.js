...
export const MASTERY = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'obs-stack', t: 'The observability stack: metrics, logs, traces — and one pane of glass', asks: "Your dashboard shows the error rate just spiked. Walk me through everything you need to find the cause, and why one signal is not enough.", d: 'Observability stands on three pillars, and each answers a question the others cannot. Metrics are numbers over time - requests per second, error rate, p99, CPU - cheap to store and perfect for dashboards and alerts; they tell you THAT something is wrong. Logs are events with detail - "10:14:03 request X for customer Y failed: connection refused" - rich but ungraphable in bulk; they tell you WHY. Traces follow one request across every service - gateway 2ms, auth 5ms, database 2,400ms - and tell you WHERE the time went, which neither metrics nor logs can show across a distributed call. The senior sentence: metrics say something is wrong, traces say where, logs say why - you need all three because each sees what the others are blind to. A collector like Prometheus scrapes and stores the metrics (as a time-series database, queried with PromQL), log and trace backends store the other two, and a tool like Grafana is the single pane of glass - it collects nothing itself, it queries and visualizes all three together so an on-call engineer sees the spike, jumps to the slow trace, and reads the failing log without leaving one screen. And you wire this BEFORE traffic arrives, because the first incident is the worst time to discover you cannot see.', go: { tpl: 'Netflix', tab: 'breakdown', do: 'Find the telemetry/metrics component in this design - picture the three pillars flowing into it and a Grafana dashboard on top, and ask what each pillar would show during an incident.' } },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'littles-law', t: "Little's Law: the one equation behind every bottleneck", asks: "A service handles 2,000 requests/sec and each takes 50ms. How many requests are in flight at once \u2014 and why does that number decide whether you fall over?", d: "Little's Law is the master equation of any queue, and it is tiny: L = \u03bb \u00d7 W. L is items in the system (concurrency), \u03bb (lambda) is the arrival rate, W is time each item spends. 2,000 req/s \u00d7 0.05s = 100 concurrent requests \u2014 so you need at least 100 workers/threads/connections in flight; with 80 a queue builds and latency climbs. It holds for ANY stable system (requests, DB connections, in-flight LLM tokens, cars on a road), turning 'we're slow' into a number. The partner equation is the queue: for M/M/1, wait W = 1 / (\u03bc \u2212 \u03bb), where \u03bc is the service rate \u2014 which explodes as \u03bb approaches \u03bc. That is the math behind 'p99 blows up near 100% utilisation': at \u03c1 = \u03bb/\u03bc = 0.9 the wait is 10\u00d7 the service time; at 0.99 it is 100\u00d7. You never schedule near \u03c1 = 1 \u2014 the equation forbids it.", go: { tpl: 'Chat (WhatsApp)', tab: 'capacity', do: "Read a tier's utilisation \u03c1; plug it into 1/(1\u2212\u03c1) and see why the last 10% of load costs far more latency than the first." } },
      { id: 'llm-inference-math', t: 'LLM inference math: prefill, decode, KV-cache and batching', asks: "Your LLM endpoint is slow and the GPUs look busy but throughput is low. Walk me through the math of WHY, and what actually fixes it.", d: "LLM inference has two phases with opposite bottlenecks, and confusing them is why people 'add a GPU' and nothing improves. PREFILL processes the whole prompt in parallel \u2014 compute-bound (many FLOPs at once), and it sets time-to-first-token. DECODE generates one token at a time, each needing the whole model's weights read from memory, so it is MEMORY-BANDWIDTH-bound: throughput \u2248 memory_bandwidth / model_bytes_per_token, and a faster-compute GPU with the same bandwidth barely helps decode. The KV-cache is the other half: each context token stores keys+values, so KV memory \u2248 2 \u00d7 layers \u00d7 heads \u00d7 head_dim \u00d7 seq_len \u00d7 batch \u00d7 bytes \u2014 it grows with sequence length \u00d7 batch and often, not the weights, caps how many requests you can batch. Batching is the lever: decode is bandwidth-bound, so many sequences per weight-read amortise the cost \u2014 throughput rises with batch size until KV memory runs out, which is exactly the throughput-vs-latency trade (bigger batch = more tokens/sec but higher per-request latency). The senior answer names the phase, cites the bound (compute vs bandwidth), and sizes the KV-cache \u2014 not 'add a GPU'.", go: { tpl: 'LLM API Platform (FastAPI)', tab: 'breakdown', do: 'Find where the LLM tier is the bottleneck; ask whether it is prefill (compute) or decode (bandwidth) \u2014 the fix differs completely.' } },
    ],
  },
  {
    ...
    items: [
      ...
      { id: 'agent-architecture', t: 'One agent or many? Single-agent vs multi-agent design', asks: "The team wants to split the AI into a crew of specialised agents. When is that the right call, and when is it just distributed-systems pain with a language model attached?", d: 'Multi-agent architectures are fashionable and usually premature. A single agent with good tools is simpler, cheaper, and easier to evaluate - reach for it first. Go multi-agent only when the work genuinely decomposes into roles that a single context window cannot hold well, or that must run in parallel: a planner that hands sub-tasks to workers, or specialists (retriever, coder, critic) with distinct tools and prompts. The catch is that the moment you have several agents talking, you have re-created every hard problem of distributed systems - coordination, partial failure, unbounded loops, ballooning token cost as agents converse - on top of models that are already non-deterministic. The senior answer names the decomposition that justifies the cost and the guardrails (step budgets, a supervisor, termination conditions) that keep a crew of agents from becoming an expensive infinite conversation.', go: { tpl: 'Multi-Agent Orchestration Platform', tab: 'breakdown', do: 'Read how the agents coordinate and where the step/cost budgets sit - then ask honestly whether a single agent with these tools would have done the job.' } },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'iac-provision-configure', t: 'Infrastructure as code: Terraform provisions, Ansible configures', asks: "Your infra is clicked together by hand in a cloud console and nobody can reproduce it. What do you reach for, and what is the difference between the two tools everyone names?", d: 'Infrastructure as code replaces console-clicking with files you version, review and re-run - and the two tools people name do opposite jobs. Terraform (or OpenTofu) PROVISIONS: it creates the infrastructure itself - servers, databases, load balancers, networks - from a declarative description of the desired end state, works out the diff between what exists and what you asked for, and makes reality match. Ansible CONFIGURES: the machines now exist, and it goes inside them to install packages, copy config, and start services. Terraform builds the house; Ansible furnishes it - provision versus configure. What makes both trustworthy is the same property your billing ledger relied on: idempotency. Running Terraform twice with the same file does nothing the second time because reality already matches the description; running an Ansible play twice does not install nginx twice. That is the whole point - an IaC tool that created three more servers on every run would be a catastrophe, so same-input-same-result-no-matter-how-many-runs is the contract, which is also what makes a failed half-run safe to simply re-run.', go: { tpl: 'Cloud-Native Gateway API Platform', tab: 'code', do: 'Open the Terraform view in the Code tab - that is the provision half (the house); picture an Ansible play configuring what runs inside each box (the furniture).' } },
  ...
  {
    id: 'frameworks', icon: '📐', title: 'Frameworks Experts Think With',
    flag: "'We do microservices and DDD and event-driven' - naming frameworks is not using them. A framework earns its place only when it changes a decision; recited as buzzwords, they are cargo-cult architecture.",
    items: [
      { id: 'c4-model', t: 'The C4 model: describe architecture at four zoom levels', asks: "Your architecture diagram is an unreadable spaghetti of 60 boxes. How do you communicate it so an exec and a new engineer each get what they need?", d: 'One diagram cannot serve everyone, and the C4 model fixes that by zooming in levels rather than cramming one view. Level 1, Context: your system as a single box among users and external systems - the picture an exec or PM needs. Level 2, Containers: the deployable/runnable units - web app, API, database, queue - the picture a new engineer needs to find their way. Level 3, Components: the major parts inside one container. Level 4, Code: classes, only where it earns it. The discipline is that each level has ONE audience and one question, so you stop drawing the god-diagram that answers none of them. Draw Context and Container for almost everything; go deeper only where a real decision hinges on it.', go: { tpl: 'Amazon (marketplace)', tab: 'hld', do: 'Read the HLD as the C4 Container level - deployable units and their calls - then imagine the one-box Context view above it for a stakeholder.' } },
      { id: 'four-plus-one', t: '4+1 views: one architecture, five lenses', asks: "Two reviewers look at the same design and argue past each other - one about deployment, one about data flow. What framework ends the argument?", d: 'The 4+1 view model says a single architecture is genuinely several diagrams for several concerns, and confusing them is why reviews go in circles. The Logical view is the domain objects and their relationships; the Process view is runtime - concurrency, what talks to what live; the Development view is code organisation - modules, layers, build; the Physical view is deployment - which box runs where. The +1 is Scenarios (use cases) that tie the four together and validate them. The senior move in a design review is naming which view you are in: "that is a Physical-view concern, we are discussing the Logical view" - it dissolves the talking-past that wastes half of architecture meetings.', go: { tpl: 'Netflix', tab: 'breakdown', do: 'The breakdown mixes several of these views - pick a paragraph and name which 4+1 view it is describing (logical? process? physical?).' } },
      { id: 'pacelc', t: 'PACELC: CAP was only half the story', asks: "Your database is up and healthy - no partition anywhere. Why might you STILL be choosing between consistency and latency?", d: 'CAP only tells you what happens during a network partition (P): pick consistency or availability. But partitions are rare, and CAP is silent about the 99.9% of the time the system is healthy - which is exactly what PACELC completes. It reads: if Partition, choose Availability or Consistency (the CAP part); Else (normal operation), choose Latency or Consistency. That Else clause is the daily reality: even with no partition, a strongly-consistent read must coordinate across replicas and pay latency, while an eventually-consistent read is fast but may be stale. So the real, everyday architectural choice is not the rare P - it is the E: how much latency will you pay for how much consistency, on every single request.', go: { tpl: 'µsvc: E-commerce (Saga)', tab: 'breakdown', do: 'Find where the saga trades immediate consistency for availability and speed - that is PACELC\'s Else-Latency branch chosen deliberately.' } },
      { id: 'twelve-factor', t: 'The Twelve-Factor App: what makes a service cloud-native', asks: "What actually has to be true about a service before it can scale horizontally and deploy safely a dozen times a day?", d: 'Twelve-Factor is the checklist that separates a service that scales cleanly from one that fights you, and a handful of its factors carry most of the weight. Config in the environment, never in code, so the same build runs in every environment. Backing services (DB, queue, cache) are attached resources addressed by URL, swappable without a code change. Processes are stateless and share-nothing, so any instance handles any request and you scale by adding processes - which is only possible because state lives in those backing services, not in memory. Disposability: start fast, shut down gracefully, so scaling and deploys are cheap. Logs are an event stream to stdout, not a file the app manages. Miss these and horizontal scaling and safe frequent deploys are simply not available to you - the factors are the preconditions for everything the cloud promises.', go: { tpl: 'SaaS AI Copilot (Multi-tenant RAG)', tab: 'capacity', do: 'The app tier scales by adding replicas - that only works because it is stateless (a Twelve-Factor process); find where state lives instead.' } },
      { id: 'ddd-bounded-context', t: 'DDD & bounded contexts: where to draw service boundaries', asks: "You are splitting a monolith into services. What decides where one service ends and the next begins - and why is 'by database table' wrong?", d: 'The hardest question in microservices is where to cut, and Domain-Driven Design answers it with the bounded context: a boundary within which a model and its language are consistent, drawn around a business capability, not a technical layer or a table. The word "customer" means something different to Billing than to Support - each is its own bounded context with its own model, and forcing one shared "Customer" across both is how you get a distributed monolith where every change touches everything. Cut services along bounded contexts and each owns its data and speaks a clear contract at the edge; cut them by table or by technical tier and you get chatty, tangled services that must be deployed together. Event storming is the workshop that finds these boundaries - map the domain events, and the contexts reveal themselves at the seams where the language changes.', go: { tpl: 'µsvc: E-commerce (Saga)', tab: 'breakdown', do: 'Each service in the saga is a bounded context owning its own data - notice none reaches into another\'s store; they coordinate by events at the edges.' } },
    ],
  },
  {
    id: 'capacity-eng', icon: '💰', title: 'Capacity & Cost Engineering (FinOps)',
    flag: "'The cluster is 90% allocated, we're full' - allocated is not used. Reserving a GPU is not running work on it, and the gap between the two is often the single largest line of waste in the whole company.",
    items: [
      { id: 'utilization-vs-allocation', t: 'Utilization vs allocation: the most expensive gap in the fleet', asks: "Your GPU cluster shows 90% allocated but the finance team asks why the bill is huge for the output. What is the distinction you have to explain, and how do you measure the real number?", d: 'Allocation is what has been RESERVED - a job asked for the GPUs and the scheduler gave them. Utilization is whether those GPUs are actually doing useful work right now. They are wildly different: a job can hold 512 GPUs and use them at 20% because it is bottlenecked on data loading, or hold them while a human debugs. On the most expensive hardware a company owns, that gap is the dominant cost, and the whole discipline of capacity engineering exists to see it and close it. You measure allocation from the scheduler and true utilization from the hardware (GPU busy %, memory, SM occupancy via DCGM-style metrics), and the number leadership actually needs is not "how full is the cluster" but "how much useful work per dollar of silicon" - occupancy is a vanity metric, goodput-per-GPU-hour is the real one.', go: { tpl: 'Nvidia (GPU Cloud Scheduler)', tab: 'capacity', do: 'The scheduler reports allocation; picture the DCGM utilization underneath it - a card can be allocated and idle, and that gap is the cost.' } },
      { id: 'capacity-planning', t: 'Capacity planning: buying the right amount of a scarce, expensive thing', asks: "How much compute should the company commit to for next year, when accelerators are scarce, expensive, and demand is uncertain?", d: 'Capacity planning is forecasting demand against a supply that has long lead times and steep pricing tiers, and being wrong in either direction is costly: under-provision and research is throttled waiting for hardware; over-provision and you have burned budget on idle silicon. The craft is separating the demand you can commit to cheaply (reserved/committed capacity, big discount, you pay whether you use it) from the demand you keep flexible (on-demand, expensive, but no commitment) - a base-plus-burst shape, where a committed base covers the steady floor and on-demand absorbs the spikes. Do it from real utilization data (what was actually USED, not what was requested), model the growth honestly, and treat the commitment as a bet you are sizing under uncertainty - too much reserved is as wrong as too little.', go: { tpl: 'Discovery Loop (Autonomous Research)', tab: 'scale', do: 'The wall here is compute scarcity; read how demand outstrips a finite pool - that is exactly what capacity planning has to forecast and commit against.' } },
      { id: 'packing-fragmentation', t: 'Packing and fragmentation: why a "half-empty" cluster is full', asks: "The cluster is only 60% allocated, yet a new 512-GPU job cannot be placed. How is that possible, and what do you do?", d: 'That is fragmentation, and it is the enemy of a scarce resource. Capacity is scattered - 8 free GPUs here, 16 there, spread across nodes - so the fleet has plenty of free cards in total but nowhere are enough of them CONTIGUOUS and topology-close for one big gang-scheduled job. The naive scheduler that hands each small job the first free GPUs quietly destroys the ability to ever place a large one. The fixes are packing (bin-pack small jobs tightly to leave large contiguous holes), defragmentation (preempt-and-repack, which needs checkpointing), and sometimes deliberately leaving a card briefly idle to preserve a big block - because keeping room for the job that matters most is worth more than filling every last GPU now. Utilization percentage alone hides this; you also have to measure whether the shape of the free space can fit the work that is queued.', go: { tpl: 'Nvidia (GPU Cloud Scheduler)', tab: 'breakdown', do: 'Read the gang-scheduling and bin-packing dive - fragmentation is exactly what packing exists to prevent.' } },
      { id: 'cloud-cost-normalization', t: 'Multi-cloud cost: making three bills comparable', asks: "You run across AWS, GCP and a third provider, each with its own billing format, discounts and instance names. How do you tell leadership what anything actually costs?", d: 'Every cloud prices and bills differently - on-demand vs reserved vs committed-use vs spot, per-second vs per-hour, opaque instance families, separate egress and storage lines, enterprise discounts applied who-knows-where. On its own each bill is unreadable and the three are incomparable, so the job is normalization: ingest the billing exports, map every provider\'s SKUs to a common model (a unit of compute, a unit of storage, a unit of egress), attribute cost back to the team and workload that incurred it, and reconcile it against what was actually used. Only then can you answer the questions that matter - cost per workload, cost per unit of output, where the committed-discount coverage is leaking, which team\'s spend is growing - and only then is "reduce cloud cost by X% without hurting the SLO" a solvable problem rather than a guess.', go: { tpl: 'Snowflake (Cloud Warehouse)', tab: 'roi', do: 'The ROI panel prices compute credits with a stated basis - imagine doing that across three providers with three billing formats; the normalization is the whole job.' } },
      { id: 'efficiency-baselines', t: 'Efficiency baselines: is this workload using the hardware it holds?', asks: "A training run holds 1,000 GPUs. How do you know if that is efficient - and what number tells you there is money on the table?", d: 'Holding the hardware is not using it well, and the discipline of efficiency is measuring how much of a workload\'s theoretical hardware capacity it actually converts into useful work - for training, roughly the fraction of peak FLOPS the model sustains (model FLOPs utilisation), and more honestly goodput: useful work done, not cycles burned. A run at 35% MFU is leaving nearly two-thirds of very expensive silicon on the floor, usually to data-loading stalls, communication overhead between GPUs, or a bad parallelism strategy. The way you drive it is per-config baselines - for this model on this hardware at this scale, what SHOULD the utilisation be - then measure the real number, find the gap, and partner with the team that owns the workload to close it. Benchmarking infrastructure that produces these baselines is what turns "the cluster feels slow" into "this run is at 35% against a 60% baseline, here is the bottleneck".', go: { tpl: 'Databricks (Lakehouse Compute)', tab: 'breakdown', do: 'The shuffle is where a distributed job wastes time; read that dive - efficiency work is finding exactly these stalls and closing the gap to the baseline.' } },
    ],
  },
]
...
export const MASTERY_CMP = {
  ...
  'agent-architecture': { cols: ['Single agent + tools', 'Multi-agent crew'], rows: [
    ['Reach for it when', 'Almost always — start here', 'Work decomposes into roles or must run in parallel'],
    ['Complexity', 'One loop, one context, easy to trace', 'Coordination, partial failure, loops — distributed systems'],
    ['Cost', 'Predictable', 'Balloons as agents converse'],
    ['Evaluation', 'Straightforward', 'Hard — which agent caused the bad output?'],
    ['Prove the split earns its cost', 'Same eval set: single-agent baseline first', 'Multi-agent must beat that baseline on held-out tasks, or it is just overhead'],
  ]},
  ...
  'obs-stack': { cols: ['Metrics', 'Logs', 'Traces'], rows: [
    ['What it is', 'Numbers over time', 'Detailed events', "One request's journey across services"],
    ['Answers', 'THAT something is wrong', 'WHY it failed', 'WHERE the time went'],
    ['Good for', 'Dashboards, alerts, trends', 'Root-cause detail', 'Cross-service latency, the slow hop'],
    ['Cost / volume', 'Cheap, compact', 'Rich but heavy in bulk', 'Sampled — you keep the interesting ones'],
    ['Typical tool', 'Prometheus (TSDB, PromQL)', 'Loki / ELK', 'Tempo / Jaeger'],
    ['One pane of glass', 'Grafana visualizes all three together', 'Grafana visualizes all three together', 'Grafana visualizes all three together'],
  ]},
  'c4-model': { cols: ['Shows', 'Audience'], rows: [
    ['1 · Context', 'Your system as one box among users + external systems', 'Execs, PMs, anyone non-technical'],
    ['2 · Container', 'Deployable/runnable units: app, API, DB, queue', 'Any engineer, onboarding'],
    ['3 · Component', 'Major parts inside one container', 'Engineers on that container'],
    ['4 · Code', 'Classes / functions', 'Only where a decision needs it'],
    ['The rule', 'One audience, one question per level', 'Draw 1 + 2 for almost everything'],
  ]},
  'four-plus-one': { cols: ['Concern it answers', 'Example artifact'], rows: [
    ['Logical', 'Domain objects and their relationships', 'Class/domain model, ER diagram'],
    ['Process', 'Runtime: concurrency, what talks to what live', 'Sequence diagram, data-flow'],
    ['Development', 'Code organisation: modules, layers, build', 'Package/module map, repo layout'],
    ['Physical', 'Deployment: which box runs where', 'Deployment/infra diagram'],
    ['+1 Scenarios', 'Use cases that tie the four together', 'Key user journeys that validate the rest'],
  ]},
  'pacelc': { cols: ['The choice', 'What it costs you'], rows: [
    ['P (partition) — rare', 'Availability OR Consistency — this is CAP', 'Only matters during an outage'],
    ['E (else, normal) — daily', 'Latency OR Consistency — what CAP ignored', 'Matters on every single request'],
    ['Strong read', 'Coordinate across replicas', 'Pay latency for a fresh answer'],
    ['Eventual read', 'Serve from the nearest replica', 'Fast, but may be stale'],
    ['The real choice', 'Usually the E, not the P', 'Tune consistency-vs-latency per request'],
  ]},
  'twelve-factor': { cols: ['Why it unlocks scale/deploys', 'Skip it and…'], rows: [
    ['Config in the environment', 'Same build runs everywhere', 'Secrets leak in code; envs drift'],
    ['Backing services as attached resources', 'Swap DB/queue/cache by URL', 'Hard-coded deps; no clean swap'],
    ['Stateless share-nothing processes', 'Any instance serves any request', 'Sticky sessions; cannot scale out'],
    ['Disposability (fast start, graceful stop)', 'Cheap scaling and deploys', 'Slow, risky rollouts'],
    ['Logs as an event stream', 'Aggregation and observability', 'Logs trapped in per-box files'],
  ]},
  'ddd-bounded-context': { cols: ['Cut by bounded context', 'Cut by table / tech layer'], rows: [
    ['Boundary is', 'A business capability + its consistent language', 'A technical layer or a database table'],
    ['"Customer" means', 'Something specific to THIS context', 'One shared model forced on everyone'],
    ['Data ownership', 'Each service owns its own data', 'Services share tables — coupled'],
    ['Result', 'Clear contracts, independent deploys', 'A distributed monolith, chatty and tangled'],
    ['Found by', 'Event storming — seams where language changes', 'Guessing from the schema'],
  ]},
  'utilization-vs-allocation': { cols: ['Allocation', 'Utilization'], rows: [
    ['What it is', 'GPUs RESERVED by a job', 'GPUs doing useful work right now'],
    ['Source', 'The scheduler', 'The hardware (DCGM: busy %, memory, occupancy)'],
    ['Can be high while', 'Cards sit idle, held by a stalled or debugging job', 'This is the number that actually earns'],
    ['Leadership metric', '"How full is the cluster" \u2014 a vanity metric', 'Goodput per GPU-hour \u2014 useful work per dollar'],
    ['The gap between them', 'Is often the single largest waste line in the company', 'Closing it is the whole discipline'],
  ]},
  'capacity-planning': { cols: ['Committed / reserved', 'On-demand'], rows: [
    ['Price', 'Big discount', 'Expensive per unit'],
    ['Commitment', 'You pay whether you use it', 'No commitment'],
    ['Covers', 'The steady floor of demand', 'The spikes above it'],
    ['Wrong when', 'You reserved more than you use \u2014 burned budget', 'You lean on it for the base \u2014 overpaying'],
    ['Plan it from', 'Real UTILISATION data, not requests', 'Honest growth modelling under uncertainty'],
  ]},
  'packing-fragmentation': { cols: ['Why it happens', 'What you do'], rows: [
    ['Big job won\'t fit at 60% allocated', 'Free cards exist but not contiguous / topology-close', 'Pack small jobs tightly to leave large holes'],
    ['First-free scheduling', 'Hands each job the nearest free GPUs', 'Bin-pack instead, preserving big contiguous blocks'],
    ['Fleet fragments over time', 'Small jobs scatter free capacity', 'Defragment: preempt and repack (needs checkpoints)'],
    ['Chasing 100% utilisation', 'Filling every last GPU strands large jobs', 'Sometimes leave one idle to keep a big block open'],
    ['Utilisation % looks fine', 'The number hides the fragmentation', 'Also measure whether the free SHAPE fits queued work'],
  ]},
  'cloud-cost-normalization': { cols: ['Raw bills', 'Normalized'], rows: [
    ['Format', 'Three providers, three billing schemas', 'One common model: compute / storage / egress units'],
    ['Pricing', 'On-demand, reserved, committed, spot \u2014 all different', 'Mapped to comparable units'],
    ['Attribution', 'Opaque \u2014 who spent what?', 'Cost tied to the team and workload that incurred it'],
    ['Answers', 'None you can trust', 'Cost per workload, per output; where discounts leak'],
    ['Unlocks', 'Guessing', '"Cut cost X% without hurting the SLO" as a real problem'],
  ]},
  'efficiency-baselines': { cols: ['Holding the hardware', 'Using it well'], rows: [
    ['The claim', '"We\'re running on 1,000 GPUs"', 'How much peak capacity becomes useful work'],
    ['Metric', 'GPUs allocated', 'MFU / goodput \u2014 sustained useful throughput'],
    ['35% MFU means', 'Two-thirds of expensive silicon on the floor', 'Money on the table'],
    ['Usual causes', '\u2014', 'Data-loading stalls, comms overhead, bad parallelism'],
    ['Driven by', 'Hoping', 'Per-config baselines \u2192 measure the gap \u2192 close it with the owner'],
  ]},
  'littles-law': { cols: ['Meaning', 'Example'], rows: [
    ['L = \u03bb \u00d7 W', 'Concurrency = arrival rate \u00d7 time in system', '2000/s \u00d7 0.05s = 100 in flight'],
    ['Need L servers', 'Fewer than L \u2192 a queue builds', '80 workers for 100 demand \u2192 latency climbs'],
    ['W = 1/(\u03bc\u2212\u03bb)', 'M/M/1 wait explodes as \u03bb \u2192 \u03bc', 'Near-full service = runaway wait'],
    ['\u03c1 = \u03bb/\u03bc = 0.9', 'Wait \u2248 10\u00d7 the service time', 'The last 10% of load is expensive'],
    ['\u03c1 = 0.99', 'Wait \u2248 100\u00d7 the service time', 'Never schedule near \u03c1 = 1'],
  ]},
  'llm-inference-math': { cols: ['Bound by', 'What actually helps'], rows: [
    ['Prefill (whole prompt)', 'Compute (FLOPs) \u2014 sets time-to-first-token', 'More compute / parallelism'],
    ['Decode (one token at a time)', 'Memory bandwidth, NOT compute', 'Higher-bandwidth GPU; batching'],
    ['Decode throughput', '\u2248 memory_bandwidth / model_bytes_per_token', 'A faster-compute GPU barely helps'],
    ['KV-cache memory', '\u2248 2\u00d7layers\u00d7heads\u00d7head_dim\u00d7seq_len\u00d7batch\u00d7bytes', 'Often caps batch size, not the weights'],
    ['Batching', 'Amortises the weight-read across sequences', 'Throughput \u2191 with batch until KV runs out (latency \u2191 too)'],
  ]},
  ...
  'iac-provision-configure': { cols: ['Terraform / OpenTofu', 'Ansible'], rows: [
    ['Job', 'Provision infrastructure', 'Configure machines that already exist'],
    ['Analogy', 'Builds the house', 'Furnishes the house'],
    ['Style', 'Declarative - describe the end state', 'Task steps - ensure this, then that'],
    ['Creates', 'Servers, databases, networks, LBs', 'Packages, files, services, users'],
    ['Idempotent because', 'It diffs desired vs actual and matches', 'Each step is "ensure", not "do again"'],
    ['Run it twice', 'Second run is a no-op - reality already matches', 'nginx is not installed twice'],
  ]},
