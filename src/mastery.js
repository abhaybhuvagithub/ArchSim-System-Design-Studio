...
export const MASTERY = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'halluc-blast-radius', t: "Hallucination as a systemic risk: its blast radius, cost, and containment", asks: "You cannot make an LLM never hallucinate. So how do you design the system so that when it DOES, it is a shrug and not an expensive incident?", d: "The mature framing is that hallucination is not a bug you eliminate, it is a fault you contain - because a probabilistic model WILL occasionally produce a confident, wrong answer, and the engineering question is what that wrong answer is allowed to touch. Its blast radius has two dimensions. COST: a hallucination that triggers a retry, a re-run, or an agent loop chasing a bad output burns tokens and compute for zero value - and at scale, hallucination-driven rework is a real line on the bill, not a rounding error. CORRECTNESS: a hallucinated fact that gets WRITTEN - into a database, an email, a downstream system, an automated action - is far worse than a wrong answer on a screen, because now the error has propagated and someone has to find and undo it. So you contain it the way you contain any fault: keep the LLM on the READ path, not the write path, until a check clears it; gate any action on a confidence signal or a verification step (retrieve-and-cite, a second check, a human for the irreversible ones); and make ABSTENTION a first-class output - 'I don't know' is a correct, cheap answer, and a system that is allowed to abstain is far safer than one forced to always answer. The senior move: treat a hallucination like a bad input that reached a boundary, and put the boundary - a check, a gate, a human - between it and anything expensive or irreversible.", go: { tpl: 'Agent Platform (Durable Execution)', tab: 'breakdown', do: 'Read the human-approval gate on irreversible actions - that is exactly the containment: a hallucinated tool call stops at a boundary before it can spend money or delete data.' } },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'tech-debt', t: 'Technical debt: the interest metaphor, and when NOT to pay it', asks: "AI tooling lets your team ship twice as fast, and the codebase is getting messier just as quickly. How do you decide what debt to pay down, and what to leave?", d: 'Technical debt is a loan, and the metaphor is exact: you borrow speed now (a shortcut, a copy-paste, a skipped abstraction) and pay INTEREST later as every future change in that area gets slower. AI coding tools have made the borrowing frictionless \u2014 you can generate a lot of code fast \u2014 which raises the debt-accrual rate, so the discipline of managing it matters MORE, not less. The senior move is to treat debt like a portfolio, not a moral failing: some debt is fine and even smart (a deliberate shortcut in a throwaway feature is a loan you never repay because you delete the feature), and some is crippling (debt in the code you change every week is high-interest and compounds). You pay down where the interest is actually being charged \u2014 the hot paths the team touches constantly \u2014 and you leave the debt in the stable corners nobody edits, because refactoring code that never changes is paying off a loan that charges no interest. The tool for large paydowns is the strangler-fig: wrap the old thing, route new work to a clean replacement, and shrink the old one over time \u2014 never a big-bang rewrite, which is a new loan with worse terms. And you make it visible: debt you cannot see (no owner, no cost, no plan) is the debt that sinks a team, so name it, estimate its interest, and put paydown on the roadmap next to features rather than pretending it is free.', go: { tpl: 'µsvc: E-commerce (Saga)', tab: 'breakdown', do: 'Imagine one service here has become the messy one everyone edits \u2014 that is where debt charges the most interest, and where a strangler-fig replacement pays off first.' } },
    ],
  },
  {
...
export const MASTERY_CMP = {
  ...
  'tech-debt': { cols: ['Pay it down', 'Leave it'], rows: [
    ['Where', 'Hot paths the team edits constantly', 'Stable corners nobody touches'],
    ['Why', 'High interest — it compounds every change', 'No interest — refactoring changes nothing'],
    ['AI angle', 'AI ships fast → debt accrues faster → manage it more', 'A shortcut in a feature you\'ll delete is a loan never repaid'],
    ['How (big paydown)', 'Strangler-fig: wrap, route new work to clean, shrink old', 'Never a big-bang rewrite — a new loan, worse terms'],
    ['Make it visible', 'Name it, estimate interest, put paydown on the roadmap', 'Invisible debt (no owner, no cost) is what sinks a team'],
  ]},
  'halluc-blast-radius': { cols: ['The risk when it hallucinates', 'The containment'], rows: [
    ['Cost', 'Retries / re-runs / agent loops chase a bad output \u2014 tokens burned for zero value', 'Cap loops + budget; a bad answer costs one call, not fifty'],
    ['Correctness', 'A wrong fact gets WRITTEN downstream \u2014 DB, email, an action', 'Keep the LLM on the READ path; gate writes behind a check'],
    ['Irreversible actions', 'Money spent, data deleted, message sent on a hallucination', 'Human-in-the-loop / verification gate before the effect'],
    ['Forced to answer', 'Confidently wrong because it must say something', 'Make ABSTENTION first-class \u2014 "I don\'t know" is correct + cheap'],
    ['Prove containment', 'You cannot trust a system you have not tested for this', 'Inject known-unanswerable inputs; the safe result is abstention, not a confident guess'],
  ]},
