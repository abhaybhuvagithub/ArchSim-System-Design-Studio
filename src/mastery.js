...
export const MASTERY = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'cache-strategies', t: 'Write-through / write-back / write-around', d: 'Through writes both places (safe, slower); back writes cache first and flushes later (fast, riskier); around skips the cache so reads warm it (good for write-heavy, rarely-read data).', go: { tpl: 'Redis (Distributed Cache)', tab: 'breakdown', do: 'Read the write-path dive and decide which policy the design implies - then argue the opposite.' } },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'lb-techniques', t: 'Round-robin vs consistent hashing', d: 'Round-robin spreads load evenly; consistent hashing keeps a key on the same node so caches stay warm and shards stay stable when the fleet changes.', go: { tpl: 'Redis (Distributed Cache)', tab: 'assist', do: 'Ask the AI: consistent hashing - the glossary answer covers why only 1/N keys move on resize.' } },
