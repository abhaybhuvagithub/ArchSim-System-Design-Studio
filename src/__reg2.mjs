...
for (const t of TEMPLATES) {
  ...
  for (const f of FAULTS) {
    ...
    const fx = compileFaults([{ fault: f, target: tgt, id: f.id }], nodes, edges)
  ...
  const d = describeArchitecture({ nodes, edges, rps: t.rps, sim, cost, sugs: review(nodes,edges,t.rps), faults: [], fx: null, template: t.name, cloud: 'aws', simOn: true })
